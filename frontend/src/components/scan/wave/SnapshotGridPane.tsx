...
export default function SnapshotGridPane({ caseId, times, autoplay = false, compact = false, controlledIdx }: Props) {
  ...
  // Tissue meshes (tooth+bone+gingiva) — visibility-critical for the
  // snapshot context, so default ON. The 1.46 MB fetch is no longer on
  // the critical path because PlotlyChart is dynamic-imported and the
  // surrounding panes paint progressively.
